...
;(function ($, window, document, undefined) {
    ...
    Tree.prototype._clickHandler = function (event) {
        ...
        var nodeIdList = target.attr('data-nodeId') ? target.attr('data-nodeId').split('\.') : [];
        var classList = target.attr('class') ? target.attr('class').split(' ') : [];
        /**
         * 根节点选择事件
         * 展开和收起菜单
         */
        if (nodeIdList.length == 2 || classList.indexOf('expand-icon') !== -1) {

            this._toggleExpanded(node, $.extend({}, _default.options));
        }
        /**
         * 普通节点的选择事件监听
         * 监听加载右侧iframe显示区、以及权限控制
         */
        else {

            /*获取页面链接 完成菜单跳转*/
            var child = target.children();
            if (child.first().hasClass('indent')) {
                ...
                if (node.selectable) {
                    if (!node)return;
                    this._setSelected(node, true, _default.options);
                }

                /**
                 * 获取显示区链接
                 */
                var hiddenInput = child.eq(3).val();

                /**
                 * 加载右侧显示区域页面
                 */
                $('#content').attr('src', hiddenInput);
